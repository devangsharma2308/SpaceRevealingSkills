...
v_values = [0.95,.9,.85,.8,.75,.7,0.65,0.6,0.55,0.5,0.45,0.4,0.35,0.3,0.25,0.2,0.15]
# v_values = [0.75,.7,0.6,0.5,0.4,0.3,0.25]
surface = 1
...
for v in v_values:
	...
	panda = panda_sim.PandaSim(p,[0,0,0],[v],surface)
	box_height = panda.box_dim[2]

	# panda.v_list = [v]
	# panda.v_list = [0.49,0.9,0.6,0.3]
	...
	#panda.save_images()
	all_v_rewards_list.append(panda.v_rewards)
	p.disconnect()
...
# plot_arm_rewards(panda.v_rewards)



# print(all_v_rewards_list)


merged_dict = {}
for d in all_v_rewards_list:
    merged_dict.update(d)

print(merged_dict)
with open("v_rewards_panda.json", "w") as f:
    json.dump(merged_dict, f)


plot_arm_rewards(merged_dict,obj_com,box_height)
